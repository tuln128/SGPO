...
import models.pretraining.model.mdlm.models as models
import models.pretraining.model.mdlm.noise_schedule as noise_schedule
import models.pretraining.model.mdlm.utils as utils
...
class MDLMDiffusion(L.LightningModule):
    def __init__(
        self,
        config,
        tokenizer):
        ...
        #self.vocab_size = self.tokenizer.K
    ...
    def forward(self, x, sigma, x_emb=None, cond=None):
        """Returns log score."""
        sigma = self._process_sigma(sigma)
        with torch.amp.autocast("cuda", dtype=torch.float32):
            logits = self.backbone(x, sigma, x_emb=x_emb, cond=cond) #might need a separate conditional if x_emb is None
    ...
    def _sample(self, num_steps=None, eps=1e-5):
        ...
        batch_size_per_gpu = self.config.eval_batch_size
    ...
    def _maybe_sub_sample(self, x0, attention_mask):
        ...
        if seqlen > self.length:
            assert seqlen == 2 * self.length
            # cropping is needed for text8-crop dataset
            # try the same starting point for now
            start = np.random.choice(self.length)
    ...
    def _reconstruction_loss(self, x0, cond=None):
       ...
       # model_output_t0 = self.forward(x0, unet_conditioning)
        model_output_t0 = self.forward(x0, unet_conditioning, cond=cond)
    ...
    def _loss(self, x0, attention_mask): #cond is not passed through anymore
