### Modified from https://github.com/enijkamp/progen2/blob/main/progen/modeling_progen.py ###
...
class ProGenForCausalLM(ProGenPreTrainedModel, GenerationMixin):
    ...
    def prepare_inputs_for_generation(self, input_ids, past=None, **kwargs):
        ...
        past = kwargs.get("past_key_values", past) #added this line to speed up generation significantly, wrong in the original progen2code
        if past:
    ...
    def forward(
        ...
        if labels is not None:
            ...
            #TODO: check if we need to ignore_index=0
            # Flatten the tokens
            ...
            ### Added log likelihood calculation for DPO ###
            # Step 1: Compute log probabilities using log_softmax
            log_probs = torch.nn.functional.log_softmax(lm_logits, dim=-1)  # Shape: (batch_size, seq_len, vocab_size)

            # Step 2: Gather log-probabilities of the target labels
            # Use labels as indices to extract probabilities
            log_likelihoods = log_probs.gather(dim=-1, index=shift_labels.unsqueeze(-1)).squeeze(-1)  # Shape: (batch_size, seq_len)

            # Step 3: Calculate sequence log-likelihoods
            sequence_log_likelihoods = log_likelihoods.mean(dim=-1)  # Mean over tokens in the sequence
        ...
        return CausalLMOutputWithPast(
            ...
            logits=lm_logits,
            log_likelihoods=sequence_log_likelihoods,
